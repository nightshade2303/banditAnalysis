...
import numpy as np
import pandas as pd
from qlfittingFunctions_20250319 import nllstickyforgetQlearning
...
for rnn in range(1, 6):
    ...
    def fun_for_pybads(x):
        return nllstickyforgetQlearning(x, data, extra_params)
    for n in range(n_optim):
        # run multiple optimizations
        print('Running optimization ' + str(n) + '...')
        options['random_seed'] = n
        bads = BADS(fun_for_pybads, None, [-0.5, -0.5, 0.0015, 0], [0.5, 0.5, 100, 1], options=options)
        ...
        with open(fr'L:\4_armed_bandit_metaRL\Unstructured_net\forgetstickyql.csv', 'a') as f:
