...
filepath = 'L:/4portProb_simulations/model_parameters_20250826/alphamat_trialsticky_first_block_25_bads'
options = {}
results = {}
tic()
for animal in animals:
    ...
    def fun_for_pybads(x):
        return nllmatQlearning_trialSticky(x, data, extra_params)
    for n in range(n_optim):
        # run multiple optimizations
        print('Running optimization ' + str(n) + '...')
        options['random_seed'] = n
        bads = BADS(fun_for_pybads, None, [-0.5, -0.5, -0.5, -0.5, 0.0015, 0], [0.5, 0.5, 0.5, 0.5, 100, 100], options = options)
