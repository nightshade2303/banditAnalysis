...
def nllforgetQlearning(x0, sessdf, arms):
    alpha_c, alpha_uc, tau, sticky, H1, H2, H3, H4 = x0
    ll = 0
...
extra_params = 4
n_optim = 5
k = 8 # number of parameters for bic
...
for animal in animals:
    ...
    def fun_for_pybads(x):
        return nllforgetQlearning(x, data, extra_params)
    for n in range(n_optim):
        # run multiple optimizationss
        print('Running optimization ' + str(n) + '...')
        options['random_seed'] = n
        
        bads = BADS(fun_for_pybads, None, [-1.0, -1.0, 0.0015, 0, 0, 0, 0, 0], [1.0, 1.0, 100, 1, 1, 1, 1, 1], options=options)
        ...
        with open('C:/Users/dlab/Desktop/alpha2Q_stickyscaledarray_v2.csv', 'a') as f:
